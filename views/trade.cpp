...
static void trade_draw_callback(Canvas* canvas, void* model) {
    ...
    if(!pokemon_fap->trading) {
        ...
        switch (pokemon_fap->gameboy_status) {
            ...
            case GAMEBOY_SEND:
                canvas_draw_icon(canvas, 38, 11, pokemon_fap->pokemon_table[pokemon_fap->curr_pokemon].icon);
                break;
            default:
...
void trade_enter_callback(void* context) {
    ...
    pokemon_fap->trade_party->party_members[0] = pokemon_fap->pokemon_table[pokemon_fap->curr_pokemon].species;
